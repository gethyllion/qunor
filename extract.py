...
parser.add_argument('-b', '--biotype', type = str, help = f'sequence biotype, protein_coding(mRNA) as default, -b A will extract all, you may choose one from the following list(your input data could contains more types that are not indicated in the list):  {biotype_types}', default='protein_coding')
...
def tar_(file = None, target_dir= 'unzipped'):
    ...
    target_dir = f'{target_dir}{hash(time.time())}'
...
def parser_json_for_sample_id(json_path = None):
    ...
    json_dict = eval(open(json_path).read().replace('null','None'))
...
if __name__ =='__main__':
    ...
    if biotype == 'A':
        print('ALL genes type selected to make matrix')
